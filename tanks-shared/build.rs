...
fn main() {
    let out_dir = PathBuf::from("src/proto_compiled");

    //let in_dir = PathBuf::from(::std::env::var("CARGO_MANIFEST_DIR").unwrap()).join("protobuffers");
    let in_dir = PathBuf::from("src/protocol_buffers");
    ...
    let config_builder = ConfigBuilder::new(&protos, None, Some(&out_dir), &[in_dir]).unwrap().dont_use_cow(true);
